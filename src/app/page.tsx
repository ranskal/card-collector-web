...
export default function HomePage() {
  ...
  const currentValue =
    openFilter === 'sport' ? sportFilter : openFilter === 'player' ? playerFilter : openFilter === 'year' ? yearFilter : ''
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="-mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-600">
        <span>Sort:</span>
        <Pill active={sortBy === 'combined'} onClick={() => setSortBy('combined')} title="Player → Year → Brand → #">
          Default
        </Pill>
        <Pill active={sortBy === 'player'} onClick={() => setSortBy('player')}>
          Player
        </Pill>
        <Pill active={sortBy === 'year'} onClick={() => setSortBy('year')}>
          Year
        </Pill>
        <Pill active={sortBy === 'brand'} onClick={() => setSortBy('brand')}>
          Brand
        </Pill>
        <Pill active={sortBy === 'number'} onClick={() => setSortBy('number')}>
          Number
        </Pill>
      </div>
      ...
      <div className="space-y-2">{/* was space-y-3 */}
        {sorted.map((c) => {
          ...
          return (
            <div key={c.id} className="card p-2">{/* was p-3 */}
              <div className="flex items-center gap-2">{/* was gap-3 */}
                <div className="relative h-14 w-14 overflow-hidden rounded-xl border border-slate-200 bg-white">{/* was h-16 w-16 */}
                  {url && (
                    <Image
                      src={url}
                      alt=""
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-semibold">{/* was text-base */}
                    {c.player?.full_name || 'Unknown Player'}
                  </div>
                  <div className="truncate text-xs text-slate-600">{/* was text-sm */}
                    {title}
                  </div>
                  <div className="mt-1">{/* was mt-2 */}
                    <span className="pill px-2 py-0.5 text-[11px]">{/* override pill size */}
                      {chip || 'Raw'}
                    </span>
                  </div>
                </div>

                <div className="flex shrink-0 items-center gap-1.5">{/* was gap-2 */}
                  <Link
                    href={`/card/${c.id}`}
                    title="Details"
                    className="rounded-lg border border-slate-300 px-2 py-1 text-xs hover:bg-slate-50" // text-xs (was text-sm)
                  >
                    Details
                  </Link>
                  <button
                    ...
                    title="Delete"
                    className="rounded-lg border border-red-300 px-2 py-1 text-xs text-red-600 hover:bg-red-50" // text-xs
                  >
                    Delete
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50" onClick={closeDialog}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 text-sm font-semibold text-slate-800">
              {openFilter === 'sport' ? 'Select Sport' : openFilter === 'player' ? 'Select Player' : 'Select Year'}
            </div>
