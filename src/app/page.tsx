...
function HomePageInner() {
  ...
  // sync state from URL (handles back/forward + cold loads)
  useEffect(() => {
  ...
  }, [sp]) // compare-before-set to avoid loops

  // keep URL in sync with state (replace; no scroll jump)
  ...
  const playerOptions = useMemo(
    () => Array.from(new Set(cards.flatMap(c => c.player?.full_name ? [c.player.full_name] : []))).sort((a,b)=>a.localeCompare(b)),
    [cards]
  )
