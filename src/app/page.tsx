...
export default function HomePage() {
  ...
  // Sync state FROM the URL (handles back/forward & deep links)
  useEffect(() => {
  ...
  const sportOptions = useMemo(
    () => Array.from(new Set(cards.flatMap((c) => (c.sport ? [c.sport] : [])))).sort(),
    [cards]
  )
  const playerOptions = useMemo(
    () =>
      Array.from(new Set(cards.flatMap((c) => (c.player?.full_name ? [c.player.full_name] : [])))).sort(
        (a, b) => a.localeCompare(b)
      ),
    [cards]
  )
  const yearOptions = useMemo(
    () => Array.from(new Set(cards.flatMap((c) => (typeof c.year === 'number' ? [c.year] : [])))).sort((a, b) => a - b),
    [cards]
  )
  const tagOptions = useMemo(
    () =>
      Array.from(
        new Set(
          cards.flatMap(
            (c) => (c.card_tags ?? []).map((ct) => ct?.tags?.label).filter(Boolean) as string[]
          )
        )
  ...
  const tagCounts = useMemo(() => {
    ...
    const by: Record<string, number> = {}
    for (const c of base)
      for (const l of (c.card_tags ?? []).map((ct) => ct?.tags?.label).filter(Boolean) as string[])
        by[l] = (by[l] ?? 0) + 1
    return { all: base.length, by }
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50" onClick={closeDialog}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-3 flex justify-end gap-2">
              {openFilter === 'tags' && (
                <button className="btn btn-outline" onClick={clearTags}>
                  Clear
                </button>
              )}
